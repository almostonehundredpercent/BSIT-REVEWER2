...
const NAME_KEY = 'bsitReviewerPlayerName';
...
function submitToGlobalLeaderboard(entry) {
  ...
  try {
    fbDb.ref('leaderboard').push(entry);
  } catch (e) {
    console.warn('Could not submit score to global leaderboard.', e);
  }
}

function loadGlobalLeaderboard(topic, callback) {
  if (!fbAvailable || !fbDb) {
    callback(null);
    return;
  }
  fbDb.ref('leaderboard').limitToLast(500).once('value')
    .then(snapshot => {
      const entries = [];
      snapshot.forEach(child => entries.push(child.val()));
      let filtered = topic === 'all' ? entries : entries.filter(e => e.topic === topic);
      // best score per player per mode+topic
      const best = {};
      filtered.forEach(e => {
      ...
      const result = Object.values(best).sort((a, b) => b.pct - a.pct || b.score - a.score);
      callback(result);
    })
    .catch(err => {
      console.warn('Could not load global leaderboard.', err);
      callback(null);
    });
}
...
function finishQuiz() {
  ...
  // Submit to global leaderboard if the player has set a name
...
// ===================== GLOBAL LEADERBOARD =====================
let leaderboardTopic = 'all';

function renderLeaderboardTopicTabs() {
  const row = $('leaderboardTopicRow');
  row.innerHTML = TOPICS.map(t => {
    const active = t.id === leaderboardTopic ? ' active' : '';
    return `<button class="toggle-btn${active}" data-topic="${t.id}">${t.label}</button>`;
  }).join('');

  row.querySelectorAll('.toggle-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      leaderboardTopic = btn.dataset.topic;
      row.querySelectorAll('.toggle-btn').forEach(b => b.classList.toggle('active', b === btn));
...
function renderGlobalLeaderboard() {
  const statusEl = $('leaderboardStatus');
  const listEl = $('leaderboardList');

  if (!fbAvailable) {
    statusEl.textContent = 'Global leaderboard is not set up yet. Add your Firebase config to firebase-config.js to enable it.';
  ...
  loadGlobalLeaderboard(leaderboardTopic, (entries) => {
    if (entries === null) {
      statusEl.textContent = 'Could not load the leaderboard. Check your connection and try again.';
      return;
    }
    if (entries.length === 0) {
      statusEl.textContent = 'No scores yet for this topic. Be the first!';
      return;
    }
    ...
    listEl.innerHTML = entries.slice(0, 50).map((e, i) => {
      const isYou = myName && e.name === myName ? ' is-you' : '';
      return `<div class="highscore-row${isYou}">
        <span class="hs-topic"><span class="hs-rank">#${i + 1}</span><span class="hs-name">${escapeHtml(e.name)}</span>
          <span style="color:var(--text-tertiary); font-weight:400;"> &middot; ${topicLabel(e.topic)} (${modeLabels[e.mode] || e.mode})</span></span>
        <span class="hs-score">${e.score}/${e.total} &middot; ${e.pct}%</span>
      </div>`;
    }).join('');
  });
}
